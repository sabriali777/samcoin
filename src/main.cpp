...
uint256 hashGenesisBlock("0x2c7311bd10d7c1f230803435476be12602f705583b7ab4d66b5ec11c9e22d404");
...
{
    // Samcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 1000); // Samcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Samcoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Samcoin: 2.5 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Samcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd065a0e98569ce6f36fb58b3f48a27cbafe19e3a132886d82d8b5af365f69d6d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1545435750;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085376055;

        if (fTestNet)
        {
            block.nTime    = 1545435737;
            block.nNonce   = 387436755;
        }
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Samcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SamcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SamcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SamcoinMiner(CWallet *pwallet)
{
    printf("SamcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("samcoin-miner");
    ...
    try { loop {
        ...
        printf("Running SamcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SamcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SamcoinMiner, pwallet));
}
